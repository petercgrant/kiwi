...
(function() {
  ...
  function encodeCodeForField(field, definitions, value) {
    switch (field.type) {
      case 'bool': {
...
// Go Compiler (tree style)
  (function() {
    ...
    function decodeCodeForField(field, definitions, value, indent) {
      var code;
      ...
      switch (field.type) {
        ...
        case 'map': {
          var keyCode = decodeCodeForField({ type: field.mapKeyType }, definitions, '&key', indent);
          if (!keyCode || kiwi.nativeTypes.indexOf(field.mapKeyType) < 0) {
            error(
              'Invalid type ' +
              quote(field.mapKeyType) +
          ...
          var mapValueField = { type: field.mapValueType, line: field.line, column: field.column };
          var valueCode = decodeCodeForField(mapValueField, definitions, '&val', indent);
          if (!valueCode) {
            error(
              'Invalid type ' +
              quote(field.mapValueType) +
    ...
    function encodeCodeForField(field, definitions, value, indent) {
      switch (field.type) {
        case 'bool': {
        ...
        case 'map': {
          var keyCode = encodeCodeForField({type: field.mapKeyType}, definitions, 'key', indent);
          if (!keyCode || kiwi.nativeTypes.indexOf(field.mapKeyType) < 0) {
            error(
              'Invalid type ' +
              quote(field.mapKeyType) +
          ...
          var mapValueField = {type: field.mapValueType, line: field.line, column: field.column};
          var valueCode = encodeCodeForField(mapValueField, definitions, 'val', indent);
          if (!valueCode) {
            error(
              'Invalid type ' +
              quote(field.mapValueType) +
        ...
        for (var i = 0; i < schema.definitions.length; i++) {
          ...
          if (pass === 0) {
            ...
            for (var j = 0; j < fields.length; j++) {
              ...
              var code = encodeCodeForField(field, definitions, value, field.isArray || field.isMap ? indent + '\t' : indent);
            ...
            for (var j = 0; j < fields.length; j++) {
              ...
              var code = decodeCodeForField(field, definitions, value, field.isArray || field.isMap ? indent + '\t' : indent);
